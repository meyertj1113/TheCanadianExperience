...
std::shared_ptr<Actor> HaroldFactory::Create(std::wstring imagesDir)
{
    ...
    auto wrench = make_shared<ImageDrawable>(L"Bowling Pin", imagesDir + L"/wrench.png");
    wrench->SetCenter(wxPoint(50, 0));
    wrench->SetPosition(wxPoint(0, 0));
    rhand->AddChild(wrench);

    actor->AddDrawable(wrench);
